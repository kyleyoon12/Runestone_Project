...
def parse():
    ...
    with open("../Customization_Service_collected_data.txt", "r", encoding="UTF-8") as file:
        raw = file.readlines()
...
def account_info(logs_samsung_account):
    ...
    cur = conn.cursor()
    cur.execute("CREATE TABLE account_info (type TEXT, collect_time TEXT, gender TEXT, birthday TEXT, country_code TEXT)")
    cur.execute("INSERT INTO account_info VALUES(?,?,?,?,?);", (logs_samsung_account[0]["type"], logs_samsung_account[0]["row_data"]["collect_time"], logs_samsung_account[0]["row_data"]["gender"], logs_samsung_account[0]["row_data"]["birthday"], logs_samsung_account[0]["row_data"]["country_code"]))
    conn.commit()
    conn.close()
